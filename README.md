...
[![Zabbix](https://img.shields.io/badge/Zabbix-6.4-red?style=flat-square&logo=zabbix&logoColor=white)](https://www.zabbix.com)
...
- [Introduction et concepts de base](#1-introduction-et-concepts-de-base)
- [Architecture de supervision recommandée](#2-architecture-de-supervision-recommandée)
- [Préparation des serveurs](#3-préparation-des-serveurs)
- [Zabbix — Supervision serveurs et réseau](#4-zabbix--supervision-serveurs-et-réseau)
- [Grafana — Tableaux de bord visuels](#5-grafana--tableaux-de-bord-visuels)
- [Netdata — Supervision temps réel](#6-netdata--supervision-temps-réel)
- [LibreNMS — Supervision réseau dédiée](#7-librenms--supervision-réseau-dédiée)
- [Antivirus — ClamAV et protection](#8-antivirus--clamav-et-protection)
- [Graylog — Centralisation des logs](#9-graylog--centralisation-des-logs)
- [Alertes et notifications](#10-alertes-et-notifications)
- [Tableaux de bord et rapports](#11-tableaux-de-bord-et-rapports)
- [Maintenance et bonnes pratiques](#12-maintenance-et-bonnes-pratiques)
- [Annexes](#-annexes)
...
|---------|-------------------|
| 🖥️ Serveurs | CPU, RAM, Disque, Température |
...
if [ "$INFECTED" -gt "0" ]; then
    echo "⚠️ ALERTE VIRUS DÉTECTÉ sur $(hostname) !" | tee -a $LOG_FILE
...
> 💡 Créer un bot Telegram : ouvrir `@BotFather` → `/newbot` → récupérer le TOKEN. Récupérer votre Chat ID via `@userinfobot`.

### 10.3 Seuils d'alerte recommandés

| Métrique | ⚠️ Attention | 🔴 Critique | 🚨 Urgence |
|----------|-------------|------------|-----------|
| CPU | > 70% | > 85% | > 95% |
| RAM | > 75% | > 90% | > 95% |
| Disque | > 70% | > 85% | > 95% |
| Charge système | > nb_cpu | > 2× nb_cpu | > 4× nb_cpu |
| Ping | > 100ms | > 500ms | Timeout |
| Perte paquets | > 1% | > 5% | > 20% |
| Bande passante | > 70% | > 85% | > 95% |
| Virus détectés | 1 | 5 | 10+ |
| Erreurs SSH | > 5/5min | > 20/5min | > 50/5min |
...
check_service() {
    if systemctl is-active --quiet $1; then
        ...
        echo "❌ $1 : ARRÊTÉ ! → Tentative de redémarrage..."
        systemctl start $1
    fi
}
...
> ⚠️ **Changer TOUS les mots de passe par défaut immédiatement !**
...
> 👁️ **Surveiller les alertes** : Une supervision non surveillée ne sert à rien.
...
**Maintenu par l'équipe informatique** • Version 1.0 • Dernière mise à jour : 2025

*Document interne — Supervision du Système d'Information Linux x86*
